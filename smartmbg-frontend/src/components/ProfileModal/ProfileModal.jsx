...
const ProfileModal = ({ isOpen, onClose, user, defaultName, defaultRole, avatarText }) => {
  ...
  const [avatarPreview, setAvatarPreview] = useState(user?.avatar ? `https://a5a1-182-8-68-206.ngrok-free.app/storage/${user.avatar}` : null);
  ...
  const handleClose = () => {
    ...
    setTimeout(() => {
      ...
      if(user?.avatar) setAvatarPreview(`https://a5a1-182-8-68-206.ngrok-free.app/storage/${user.avatar}`);
      onClose();
